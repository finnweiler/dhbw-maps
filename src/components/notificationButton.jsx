...
class NotificationButton extends React.Component {
  ...
  async notify_check() {
    ...
    await navigator.permissions
      ...
      .then((permission) => (permission_state = permission.state))

    // send a message based on the permission state
    if (permission_state === 'denied') {
      this.notify(
        'Du hast den Standortzugriff verweigert!',
  ...
  async send_activated_notfication(max_tries = 5) {
    let return_value = await this.notify(
  ...
  render() {
    ...
    return (
      <div style={{ zIndex: 1000, position: 'absolute', right: '55px', bottom: '25px' }}>
        <Button fill raised onClick={() => this.toggle_notifications()}>
